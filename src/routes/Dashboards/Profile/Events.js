...
import NoRecordFound from 'routes/Dashboards/Crm/NoRecordFound';
...
const EventItem = ({ item, onPress }) => {
  ...
  return (
    <CmtMediaObject
      avatar={<CmtImage className="pointer" src={item.url} alt={item.name} />}
      avatarPos="center"
      title={getTitle()}
...
const Events = ({ events, onOpen }) => {
  const classes = useStyles();
  const _handleOpen = item => {
    onOpen && onOpen(item);
  };
  return (
    <CmtCard>
      <CmtCardHeader title="Dự án đã tạo" subTitleProps={{ className: classes.subTitleRoot }} />
      <CmtCardContent>
        <CmtList
          data={events}
          renderRow={(item, index) => (
            <ListItem className={classes.eventItem} component="div" key={index}>
              <EventItem item={item} onPress={_handleOpen} />
            </ListItem>
          )}
        />
        {!events.length > 0 && <NoRecordFound>Không có dữ liệu.</NoRecordFound>}
      </CmtCardContent>
    </CmtCard>
  );
};
